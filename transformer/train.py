...
def greedy_decode(model, source, source_mask, token_src, token_tgt, max_len, device):
    sos_idx = token_tgt.token_to_id('[SOS]')
    ...
    while True:
        ...
        prob = model.linear_layer(decoder_output[:, -1])
...
def run_validation(model, validation_ds, tokenizer_src, tokenizer_tgt, seq_len, device, log_fn, global_step, writer, num_examples=2):
    model.eval()
    count = 0
    ...
    with torch.no_grad():
        for batch in validation_ds:
            ...
            model_out = greedy_decode(model, encoder_input, encoder_mask, seq_len, tokenizer_tgt.get_vocab_size(), device)
...
def get_ds(config):
    ...
    #dataset split
    train_ds = int(0.9 * len(ds_raw))
    val_ds = len(ds_raw) - train_ds
    train_ds_raw, val_ds_raw = random_split(ds_raw, [train_ds, val_ds])
...
def train(config):
    ...
    if model_filename:
        ...
        print('No model preloaded, staring from scratch')
